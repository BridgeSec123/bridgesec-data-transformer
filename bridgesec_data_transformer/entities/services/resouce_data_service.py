...
class EntityDataService:
    ...
    def fetch(self, date_str, entity_name):
        logger.info(f"Fetching data for date: {date_str}, entity: {entity_name}")

        db_name = get_latest_db(self.mongo_client, date_str)
        logger.info(f"Found database: {db_name}")
        if not db_name:
            logger.warning(f"No database found for date: {date_str}")
            return []

        db = self.mongo_client[db_name]

        if entity_name == "Auth Server":
            logger.info("Using AuthServerDataBuilder for nested data")
            return AuthServer().build(db)

        if entity_name == "Policy MFA":
            logger.info("Using PolicyMFADataBuilder for nested data")
            return PolicyMFADataBuilder().build(db)

        if entity_name == "App Signon Policy":
            logger.info("Using AppSignonPolicyDataBuilder for nested data")
            return AppSignonPolicyDataBuilder().build(db)

        if entity_name == "Policy Password":
            logger.info("Using PolicyPasswordDataBuilder for nested data")
            return PolicyPasswordDataBuilder().build(db)

        if entity_name == "Policy Profile Enrollment":
            logger.info("Using PolicyEnrollmentDataBuilder for nested data")
            return PolicyEnrollmentDataBuilder().build(db)

        if entity_name == "Policy Sign On":
            logger.info("Using PolicySignonDataBuilder for nested data")
            return PolicySignonDataBuilder().build(db)

        if entity_name == "Admin Role Custom":
            logger.info("Using AdminRoleCustomDataBuilder for nested data")
            return AdminRoleCustomDataBuilder().build(db)

        collection_name = get_collection_name(entity_name)
        logger.info(f"Collection name for {entity_name}: {collection_name}")
        if not collection_name:
            logger.warning(f"No collection name found for entity: {entity_name}")
            return []

        result = list(db[collection_name].find({}, {"_id": 0}))
        logger.info(f"Found {len(result)} records in collection {collection_name}")
        return result